...
class PdfJinja(object):
    ...
    def register_filters(self):
        self.jinja_env.filters.update(dict(
            ...
            paste=self.paste,
            check=self.check,
            X=lambda v: "On" if v else "Off",
            Y=lambda v: "On" if v else "Off",
        ))
    ...
    def paste(self, data):
        ...
        pdf = self.Attachment(data, dimensions=(centerX, centerY, resizeImgW, resizeImgH)).pdf()
        ...
        for ref in widgets:
            ...
            if "TU" in ref:
                ...
                try:
                    ...
                    if tmpl.startswith('{{') and tmpl.endswith('}}'): # important if you want to not fill the pdf with crap
    ...
    def exec_pdftk(self, data, flatten):
        print('here we go with data = = = ', data)
        fdf = forge_fdf("", data.items(), [], [], [], checkbox_checked_name="On")
        args = [
            "pdftk",
            self.filename,
            "fill_form", "-",
